# Powerlevel10k — Nord Flat
# Calm two-line Nord prompt with no background blocks and no line connectors.
...
() {
  ...
  # Nord palette: Frost, Snow Storm and Aurora status colors.
  typeset -g P10K_NORD_POLAR='#4C566A' P10K_NORD_FROST='#88C0D0'
  typeset -g P10K_NORD_FROST_2='#81A1C1' P10K_NORD_SNOW='#ECEFF4'
  typeset -g P10K_NORD_GREEN='#A3BE8C' P10K_NORD_YELLOW='#EBCB8B'
  typeset -g P10K_NORD_RED='#BF616A'

  # Information on line one, command input on line two.
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
    os_icon dir vcs node_version pyenv virtualenv rust_version docker_context
    newline prompt_char
  )
  ...
  # Flat transparent styling with all connector prefixes disabled.
  ...
  function p10k_nord_git_formatter() {
    emulate -L zsh
    if [[ -n $P9K_CONTENT ]]; then typeset -g P10K_NORD_GIT=$P9K_CONTENT; return; fi
    local meta="%F{$P10K_NORD_FROST_2}" ok="%F{$P10K_NORD_GREEN}"
    local warn="%F{$P10K_NORD_YELLOW}" bad="%F{$P10K_NORD_RED}" r branch
    if (( ! $1 )); then meta="%F{$P10K_NORD_POLAR}"; ok=$meta; warn=$meta; bad=$meta; fi
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      branch=${(V)VCS_STATUS_LOCAL_BRANCH}; (( $#branch > 32 )) && branch[13,-13]='…'
    ...
    typeset -g P10K_NORD_GIT=$r
  }
  functions -M p10k_nord_git_formatter 2>/dev/null
  typeset -g POWERLEVEL9K_VCS_DISABLE_GITSTATUS_FORMATTING=true
  typeset -g POWERLEVEL9K_VCS_CONTENT_EXPANSION='${$((p10k_nord_git_formatter(1)))+${P10K_NORD_GIT}}'
  typeset -g POWERLEVEL9K_VCS_LOADING_CONTENT_EXPANSION='${$((p10k_nord_git_formatter(0)))+${P10K_NORD_GIT}}'
  ...
  # Active development runtimes.
  typeset -g POWERLEVEL9K_NODE_VERSION_FOREGROUND=$P10K_NORD_FROST
  typeset -g POWERLEVEL9K_NODE_VERSION_PROJECT_ONLY=true
  typeset -g POWERLEVEL9K_NODE_VERSION_VISUAL_IDENTIFIER_EXPANSION=''
  typeset -g POWERLEVEL9K_PYENV_FOREGROUND=$P10K_NORD_FROST_2
  typeset -g POWERLEVEL9K_PYENV_SOURCES=(shell local)
  typeset -g POWERLEVEL9K_PYENV_SHOW_SYSTEM=false
  typeset -g POWERLEVEL9K_PYENV_VISUAL_IDENTIFIER_EXPANSION=''
  typeset -g POWERLEVEL9K_VIRTUALENV_FOREGROUND=$P10K_NORD_FROST_2
  typeset -g POWERLEVEL9K_VIRTUALENV_SHOW_PYTHON_VERSION=true
  typeset -g POWERLEVEL9K_VIRTUALENV_SHOW_WITH_PYENV=if-different
  typeset -g POWERLEVEL9K_VIRTUALENV_VISUAL_IDENTIFIER_EXPANSION=''
  typeset -g POWERLEVEL9K_RUST_VERSION_FOREGROUND=$P10K_NORD_FROST
  typeset -g POWERLEVEL9K_RUST_VERSION_PROJECT_ONLY=true
  typeset -g POWERLEVEL9K_RUST_VERSION_VISUAL_IDENTIFIER_EXPANSION=''

  # Docker context with a five-second cache.
  typeset -g P10K_NORD_DOCKER_CACHE=
  typeset -gi P10K_NORD_DOCKER_AT=-10
  function prompt_docker_context() {
    emulate -L zsh
    (( $+commands[docker] )) || return
    local ctx=$DOCKER_CONTEXT
    if [[ -z $ctx ]]; then
      if (( SECONDS - P10K_NORD_DOCKER_AT >= 5 )); then
        P10K_NORD_DOCKER_CACHE=$(command docker context show 2>/dev/null)
        P10K_NORD_DOCKER_AT=$SECONDS
      fi
      ctx=$P10K_NORD_DOCKER_CACHE
    fi
    [[ -n $ctx ]] && p10k segment -f $P10K_NORD_FROST_2 -i '' -t "${ctx//\%/%%}"
  }

  # Prompt character and right-side status.
  ...
  typeset -g POWERLEVEL9K_STATUS_{ERROR,ERROR_SIGNAL,ERROR_PIPE}_FOREGROUND=$P10K_NORD_RED
